...
function App() {
  ...
  return <main className="desktop"><div className="frame">
    ...
    <div className="stats">
      ...
      <Readout icon={netFlow24h >= 0n ? ArrowUpFromLine : ArrowDownToLine} label="24-hour net flow" value={`${netFlow24h >= 0n ? '+' : '−'}${amount(netFlow24h >= 0n ? netFlow24h : -netFlow24h, 2)} NET`} sub="Adds minus removals" change={{ text: netFlow24h > 0n ? 'Net staking growth' : netFlow24h < 0n ? 'Net staking outflow' : 'No net change', tone: netFlow24h > 0n ? 'positive' : netFlow24h < 0n ? 'negative' : 'idle' }} />
      ...
      <Readout icon={Coins} label="WinNET lottery stake" value={`${amount(venueStake.winNet, 2)} sNET`} sub={`${winNetParticipants.toLocaleString()} active principal wallets${fund?.playingTonight != null ? ` · ${fund.playingTonight.toLocaleString()} playing tonight` : ''}`} />
      <Readout icon={Coins} label="wsNET wrapper backing" value={`${amount(venueStake.wsNet, 2)} sNET`} sub="Wrapped staking contract · separate from the WinNET prize vault" />
