...
function App() {
  ...
  const rows = useMemo(() => {
    ...
    const direct = data.stakers.filter((row) => !excludedAddresses.has(row.address.toLowerCase())).map((row) => ({ ...row, venue: 'Direct', lotteryWinnings: '0' }));
