...
function App() {
  ...
  return <main className="desktop"><div className="frame">
    ...
    <div className="stats">
      ...
      <Readout icon={Users} label="True unique holders" value={trueHolderCount?.toLocaleString() || 'Indexing…'} sub={latestMetrics?.trueHolderCount ? `${latestMetrics.directHolderCount.toLocaleString()} direct · ${latestMetrics.winNetHolderCount.toLocaleString()} WinNET · ${latestMetrics.wsNetHolderCount.toLocaleString()} wsNET · ${latestMetrics.holderOverlapsRemoved.toLocaleString()} overlaps removed · ${trueStakerCount.toLocaleString()} stakers (${addressStakingPct.toFixed(1)}%)` : 'Combining direct and contract-underlying holders'} change={change24h(trueHolderCount, baseline24h?.trueHolderCount)} />
