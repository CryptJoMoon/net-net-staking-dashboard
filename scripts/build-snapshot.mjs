...
async function collectMetrics(state) {
  ...
  const rwaSleeveUsd = Array.isArray(sleeve) ? sleeve.filter((item) => sleeveTokens.has(item.token?.address_hash?.toLowerCase()) && item.token?.exchange_rate).reduce((sum, item) => sum + Number(item.value) / 10 ** Number(item.token.decimals) * Number(item.token.exchange_rate), 0) : null;
